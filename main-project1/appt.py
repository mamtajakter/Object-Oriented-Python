...
class Appt:
    ...
    def intersect(self, other:"Appt")-> "Appt":
        ...
        return Appt(max(self.start, other.start), min(self.finish, other.finish), str(self.desc)+ " & " + str(other.desc) )
...
class Agenda:
    ...
    def conflicts(self)->"Agenda":
        """Returns an agenda consisting of the conflicts
                (overlaps) between appointments in this agenda and the otehr.
           Side effect: This agenda is sorted
        """
        _size = len(self.elements)
        # x= self.elements
        self.sort()
        ...
        for i in range(_size):
            ...
            for j in range(k, _size):
                ...
                if self.elements[i].overlaps(self.elements[j]):
                    ...
                    repeated.append(self.elements[i].intersect(self.elements[j]))
                    # repeated.append(self.elements[i])
                    # repeated.append(self.elements[j])
        # print(repeated)
        return repeated
